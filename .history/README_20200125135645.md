...
> Answer: Benefits of client-side routing include quicker/non existent load times for animations and transitions. The ability for the request to be handled by Javascript instead of sending a request to the server reduces the need to wait for a server to respond to a request, thus giving almost instant renderings and reactions to user requests. 
