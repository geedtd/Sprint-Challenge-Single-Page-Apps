...
> Answer: One of the benefits of client side routing is that the whole webpage is not requested from the server. Instead Javascript handles the request for a new rendering of the webpage or component. This allows a quicker render since nothing is fetched from a server, instead it is handled by the existing Javascript. Client side routing also allows for smoother transitions and animations in the website it is being used on, hence enhancing appearance. 
